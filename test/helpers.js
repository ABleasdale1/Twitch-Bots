...
function loadModule(relative, mocks = {}, globals = {}) {
  ...
  const context = {
    ...
    __filename: filename,
    require: (name) =>
      Object.hasOwn(mocks, name) ? mocks[name] : nativeRequire(name),
    console: {
      log() {},
      warn() {},
      error() {},
...
function fakeTimers() {
  ...
  return {
    ...
    setTimeout: (fn, ms) => {
      const key = ++id;
      jobs.set(key, { fn, ms, interval: false });
      return key;
    },
    clearTimeout: (key) => jobs.delete(key),
    setInterval: (fn, ms) => {
      const key = ++id;
      jobs.set(key, { fn, ms, interval: true });
      return key;
    },
...
async function joinHarness() {
  ...
  const context = {
    ...
    process: {
      env: {
        ...
        JOIN_USERNAME: "testjoin",
        ACCESS_TOKEN: "fake",
        JOIN_ACCESS_TOKEN: "fake",
        CLIENT_ID: "fake",
        BROADCASTER_ID: "fake",
      },
      stdin: { isTTY: false },
    ...
    require(name) {
      ...
      if (name === "./tokenManager") {
        return {
          ensureValidToken: async () => ({ accessToken: "fake" }),
        };
      }

      if (name === "./utils/logger") {
        return { setStatusLine() {} };
      }

      if (name === "./utils/twitchApi") {
        return {
          isStreamLive: (...args) => liveCheck(...args),
        };
      }

      if (name === "./utils/botRuntime") {
        return {
          requireSingleInstance() {},
          safeAsync: (prefix, label, handler) =>
            (...args) =>
              Promise.resolve()
                .then(() => handler(...args))
  ...
  vm.runInNewContext(
    fs.readFileSync(path.join(root, "joinBot.js"), "utf8") +
      `
  ...
  return {
    ...
    get host() {
      return clients.find((client) => client.username === "testbot");
    },
    get joiner() {
      return clients.find((client) => client.username === "testjoin");
    },
  };
}
