...
for (const reenable of [false, true]) {
  const suffix = reenable ? " even if immediately re-enabled" : "";

  test(`ar off cancels an in-flight live check${suffix}`, async () => {
    const bot = await joinHarness();
    const live = deferred();
...
for (const offCommand of ["ar off", "aj off"]) {
  test(`${offCommand} cancels a pending personal raffle join`, async () => {
    ...
    const joinTimer = [...bot.timers.jobs].find(
      ([, job]) => job.ms <= 58_000
    )?.[0];
