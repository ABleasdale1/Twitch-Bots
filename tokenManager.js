...
const minimumTokenLifetimeSeconds = 5 * 60;
const checksInFlight = new Map();

function getProfile(profileName) {
  const profile = tokenProfiles[profileName];
...
function updateEnvValue(content, key, value) {
  ...
  return `${content}${newline}${key}=${safeValue}\n`;
}
...
async function validateToken(accessToken) {
  ...
  try {
    ...
    if (!Number.isFinite(Number(data.expires_in))) {
      throw new Error("missing token lifetime");
    }
  } catch (error) {
    return {
      valid: null,
...
async function performRefresh(profileName, profile) {
  ...
  if (!data.access_token) {
    throw new Error("Twitch refresh response contained no access token");
  }
  ...
  fs.writeFileSync(temporaryEnvPath, envContent, {
    encoding: "utf8",
    mode: 0o600,
  });
...
async function refreshTokens(profileName = "bot") {
  ...
  const lock = await acquireFileLock(
    path.join(__dirname, ".token-refresh.lock")
  );
  ...
  try {
    ...
    const currentAccessToken = process.env[profile.accessTokenKey];
    const validation = await validateToken(currentAccessToken);

    if (
      validation.valid === true &&
      validation.expiresIn > minimumTokenLifetimeSeconds
    ) {
      return {
        accessToken: currentAccessToken,
        refreshToken: process.env[profile.refreshTokenKey],
        refreshed: false,
        validated: true,
...
async function checkToken(profileName = "bot") {
  ...
  const accessToken = process.env[profile.accessTokenKey];
...
function ensureValidToken(profileName = "bot") {
  if (!checksInFlight.has(profileName)) {
    checksInFlight.set(
      profileName,
      checkToken(profileName).finally(() => checksInFlight.delete(profileName))
    );
  }
