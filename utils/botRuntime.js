...
function requireSingleInstance(botName, channel) {
  ...
  const key = crypto.createHash("sha256")
    .update(`${botName}:${String(channel).replace(/^#/, "").toLowerCase()}`).digest("hex").slice(0, 24);
  const lock = tryFileLock(path.join(directory, `${key}.lock`));
  if (!lock) {
    console.error(`[${botName}] Another copy is already running for this channel. Stop the extra manual/service copy. Exit code 73.`);
    process.exit(73);
  }
...
function createChatClient({ username, channel, profile }) {
  return new tmi.Client({
    ...
    identity: {
      ...
      password: async () => `oauth:${(await ensureValidToken(profile)).accessToken}`,
    },
    channels: [channel],
  });
}
...
function startBotRuntime({
  prefix, connections, onFatal,
  healthTimeoutMs = 180_000, healthCheckMs = 15_000,
  ...
  const normalise = (channel) => String(channel).replace(/^#/, "").toLowerCase();
  const states = connections.map(({ client, channel, profile }) => ({
    client, channel: normalise(channel), profile,
    connected: false, joined: false, lastPong: now(), unhealthySince: now(),
  }));
  const listeners = [];
  ...
  for (const state of states) {
    ...
    listen(state.client, "join", (channel, username, self) => { if (self) joined(channel); });
    listen(state.client, "pong", () => { state.lastPong = now(); });
    listen(state.client, "disconnected", (reason) => {
      state.connected = false;
      state.joined = false;
      if (state.unhealthySince === null) state.unhealthySince = now();
      setStatusLine(prefix, `Connection ${state.profile}`, `Disconnected: ${reason || "unknown"}; waiting for reconnect`);
    });
    ...
    listen(state.client, "error", (error) => {
      setStatusLine(prefix, "Error", `Chat ${state.profile}: ${error?.message || error}`);
    });
  }
  const healthTimer = setIntervalFn(() => {
    for (const state of states) {
      if (state.connected && state.joined && state.client.readyState() === "OPEN") {
        if (now() - state.lastPong >= healthTimeoutMs) {
          fail(`No Twitch heartbeat for ${Math.round(healthTimeoutMs / 1000)}s (${state.profile}); restarting bot`);
          return;
        }
        state.unhealthySince = null;
      } else {
        ...
        if (now() - state.unhealthySince >= healthTimeoutMs) {
          fail(`Chat unavailable for ${Math.round(healthTimeoutMs / 1000)}s (${state.profile}); restarting bot`);
          return;
        }
      }
    }
  ...
  function checkTokens() {
    ...
    maintenance = (async () => {
      for (const profile of new Set(states.map((state) => state.profile))) {
        ...
        if (stopped) return;
        setStatusLine(prefix, `Token ${profile}`, result.temporaryFailure
      ...
      fail(`Token maintenance failed: ${error?.message || error}; restarting bot`);
  ...
  function stop() {
    ...
    for (const [client, event, handler] of listeners) client.removeListener(event, handler);
  }
  return { stop, checkTokens };
}

module.exports = { requireSingleInstance, createChatClient, safeAsync, startBotRuntime };
